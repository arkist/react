...
[npm](https://www.npmjs.com/) 모듈을 가지고 있다면, `npm install -g babel`를 실행할 수 있습니다. Babel은 React v0.12, v0.13를 내장 지원합니다. 태그는 자동적으로 동등한 `React.createElement(...)`로 변환되며, `displayName`은 모든 React.createClass 호출에 자동으로 추론, 추가됩니다.
